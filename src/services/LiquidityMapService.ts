...
export class LiquidityMapService {
    ...
    private detectEmptyZones(clusteredLevels: ClusteredLevel[], minPrice: number, maxPrice: number, threshold: number): { start: number; end: number }[] {
        ...
        if (clusteredLevels.length === 0) return [];
    ...
    public async generateLiquidityMap(symbol: string): Promise<Buffer | null> {
        ...
        const createLevelAnnotation = (price: number, volume: number, type: 'long' | 'short', totalVolume: number, isTop: boolean): AnnotationOptions => {
        ...
        const topPrices = [...topLongPrices, ...topShortPrices];
        const allLevelAnnotations = [
            ...longsData.map(l => createLevelAnnotation(l.price, l.volume / 1000, 'long', totalVolume, topPrices.includes(l.price))),
            ...shortsData.map(l => createLevelAnnotation(l.price, l.volume / 1000, 'short', totalVolume, topPrices.includes(l.price)))
        ];
        ...
        const emptyZoneAnnotations: AnnotationOptions[] = [];
